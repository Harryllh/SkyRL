...
# Colocated GRPO training for safety classification using Nemotron-Content-Safety-Reasoning-4B.
#
# Step 1: prepare data
...
#   bash examples/train/safety/run_nemotron_thinking.sh
#
# Override defaults:
#   NUM_GPUS=8 bash examples/train/safety/run_nemotron_thinking.sh
...
uv run --isolated --python 3.12 --extra fsdp -m skyrl.train.entrypoints.main_base \
  ...
  trainer.run_name="nemotron_content_safety_reasoning_4b_grpo_fsdp2_lora_r${LORA_RANK}_a${LORA_ALPHA}" \
  trainer.resume_mode=latest \
  trainer.ckpt_path="$HOME/ckpts/nemotron_content_safety_reasoning_4b_lora_r${LORA_RANK}_a${LORA_ALPHA}_ckpt" \
  trainer.hf_save_interval=2 \
  trainer.export_path="$HOME/hf_ckpt/nemotron_content_safety_reasoning_4b_lora_r${LORA_RANK}_a${LORA_ALPHA}" \
  $@
